...
**Nudge is the application layer on top of Shotoku** — the dashboard, ledger, and policy interface a human actually uses. It is the commercial product in an open-core model (the Shotoku SDK, proxy, and policy engine are open source).

Nudge began as a standalone headless component library; those components (`ApprovalQueue`, `AuditTrail`) now live on as internal screens of the app rather than as the deliverable. See [`docs/PRODUCT.md`](docs/PRODUCT.md) for the full product context.
...
```
nudge/
├── client/     @nudge/client — React 19 + TS + Tailwind v4 web dashboard (Vite)
├── shared/     @nudge/shared — domain types + audit hash-chain, shared across frontends
...
- **`shared/`** holds the domain model and the audit hash-chain (`hashEntry` / `verifyChain`), imported by the client as `@nudge/shared`. It's the seam that the future Tauri desktop app will also build on.
...
All scripts run from the repository root and delegate to `@nudge/client`.
